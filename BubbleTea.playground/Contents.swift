...
let milk = LiquidType(color: .white, image: NSImage(named: "Milk.png"))
let sugar = LiquidType(color: CGColor(red: 1, green: 1, blue: 1, alpha: 0.8), image: NSImage(named: "Sugar.png"))
let vanilla = LiquidType(color: CGColor(red: 0.3, green: 0.15, blue: 0, alpha: 0.97), image: NSImage(named: "Vanilla.png"))
let strawberry = LiquidType(color: CGColor(red: 1.0, green: 0.4, blue: 0.7, alpha: 1.0), image: NSImage(named: "Strawberry.png"))
let mango = LiquidType(color: CGColor(red: 1.0, green: 0.8, blue: 0.5, alpha: 1.0), image: NSImage(named: "Mango.png"))
let banana = LiquidType(color: CGColor(red: 1.0, green: 1.0, blue: 0.7, alpha: 1.0), image: NSImage(named: "Banana.png"))
...
for liquid in [strawberry, mango, banana, tea, milk, sugar, vanilla] {
    ...
    dispenser.position = SCNVector3(pos, 6, -2)
...
pos = 3
for bubble in [tapioca, redBean, aloe] {
    let bubbleDispenser = BubbleBoxNode(bubbleType: bubble)
    bubbleDispenser.position = SCNVector3(pos, 3, 0)
    scene.rootNode.addChildNode(bubbleDispenser)
    pos += 1
}

manager.loadLevel(Level(scene: scene, liquids: [tea, milk, sugar, vanilla, strawberry, mango, banana], maxLiquidsPerOrder: 7, bubbles: [tapioca, redBean, aloe], maxBubblesPerOrder: 3, orderFrequency: 1, name: "Test Level"))
